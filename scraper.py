...
def scraper(list_of_urls_mena, list_of_urls_int, list_of_urls_proc):
    ...
    for url in list_of_urls_mena:
        article = newspaper.Article(url="%s" % (
            url), language='en', config=config)
        try:
            article.download()
            article.parse()
            time.sleep(2)
        ...
        if article.publish_date is not None:
            publish_date = datetime.strptime(
                str(article.publish_date), '%Y-%m-%d %H:%M:%S').strftime('%d %B %Y')
        obj = {'title': article.title, 'top_image': article.top_image,
               'publish_date': publish_date, 'url': article.url}
    ...
    sorted(extracted1['articles'], key=lambda d: d['publish_date'])
    sorted(extracted2['articles'], key=lambda d: d['publish_date'])
    sorted(extracted3['articles'], key=lambda d: d['publish_date'])

    return extracted1, extracted2, extracted3


# if __name__ == "__main__":
#     print(scraper(list_of_urls_mena, list_of_urls_int, list_of_urls_proc))
